...
namespace LH.GOAP
{
    public class Node 
    {
        ...
        public Dictionary<string, int> stateDict;
        ...
        public Node(Node parent, float cost, Dictionary<string, int> allStates, GAction action) {
            this.parent = parent;
            this.cost = cost;
            this.stateDict = new Dictionary<string, int>(allStates);
            this.action = action;
        }
        ...
        public Node(Node parent, float cost, Dictionary<string, int> allStates, Dictionary<string, int> beliefStates, GAction action) {
            ...
            foreach (KeyValuePair<string, int> b in beliefStates) {
                if (!this.stateDict.ContainsKey(b.Key)) {
                    this.stateDict.Add(b.Key, b.Value);
                }
            }
            this.action = action;
        }
    ...
    {
        public Queue<GAction> Plan(List<GAction> actions, Dictionary<string, int> goal, GStates beliefStates) {

        List<GAction> usableActions = new List<GAction>();
        //of all the actions available find the ones that can be achieved.
        foreach (GAction a in actions) {
            if (!a.IsAchievable()) {
                Debug.Log("Action not achievable: " + a.actionName);
                continue;
            }
        ...
        Node start = new Node(null, 0.0f, GWorld.Instance.GetWorldStates().GetStates(), beliefStates.GetStates(), null);
    ...
    private bool BuildGraph(Node parent, List<Node> leaves, List<GAction> usableActions, Dictionary<string, int> goal) {
        ...
        foreach (GAction action in usableActions) {
            ...
            if (action.IsAchievableGiven(parent.stateDict)) {

                //get the state of the world if the parent node were to be executed
                Dictionary<string, int> currentStateDict = new Dictionary<string, int>(parent.stateDict);
                ...
                foreach (KeyValuePair<string, int> eff in action.effectsDict) {
                    if (!currentStateDict.ContainsKey(eff.Key)) {
                        currentStateDict.Add(eff.Key, eff.Value);
                    }
                }

                //create the next node in the branch and set this current node as the parent
                Node node = new Node(parent, parent.cost + action.cost, currentStateDict, action);
                ...
                if (GoalAchieved(goal, currentStateDict)) {
                    leaves.Add(node);
                    foundPath = true;
                } else {
